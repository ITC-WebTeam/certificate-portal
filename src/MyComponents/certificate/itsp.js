...
export default function ITSP() {
  ...
  const downloadfile = async () => {
    ...
    if (ismentee) {
      ...
      const link = `${process.env.PUBLIC_URL}certificates/completion/${key}.png`;
      await saveAs(link, "itsp-certificate.png");
    } else if (ismentor) {
      var mentor = document.getElementById("mentor").value;
      key = mentor.replace(/\s+/g, "%20");
      const link = `${process.env.PUBLIC_URL}/certificates/Mentors/${key}.png`;
  ...
  return (
    <div>
      ...
      <div className="button">
        {isselected ? <button className="btn btn-danger" onClick={downloadfile}>Download</button> : <></>}
      </div>
    </div>
  );
}
